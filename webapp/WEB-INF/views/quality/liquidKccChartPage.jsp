...
meq_kcc, ash_kcc, nv_kcc, ph_kcc, conductivity_kcc;
...
function fetchData() {
    $.ajax({
        type:"POST",
        url:"/geomet/quality/getKccChart",
        data:{},
        success:function(result){
            ...
            meq_kcc=uniqueOrder.map(rt=>safeNum(grouped[rt].meq_kcc));

            const newSeriesData=[ meq_result, meq_kcc];
...
function getTrend(){
	kccMeqChart = Highcharts.chart('kccMeqChart',{
        chart:{ type:'line' },
        title:{ text:'MEQ(mmol)<br>(Spec.36~45)' },
        plotOptions: {
        ...
        yAxis:[
            { title:{ text:"결과값", rotation:0 }, labels:{ align:"right", x:-10 }, 
              min:0, max:54.0, 
              tickAmount: 9,   
              tickPositions: [30, 33, 36, 39, 42, 45, 48, 51, 54],          
              endOnTick: false,
              showLastLabel: true,
              plotLines: [
                  {
        ...
        series:[
            { name:'진합', data:meq_result, yAxis:0 },
            { name:'KCC', data:meq_kcc, yAxis:0 }
        ]
    });
}
...
function fetchData2() {
    $.ajax({
        ...
        success:function(result){
            ...
            ash_kcc=uniqueOrder.map(rt=>safeNum(grouped[rt].ash_kcc));

            const newSeriesData=[ ash_result, ash_kcc];
	...
	kccAshChart = Highcharts.chart('kccAshChart',{
        ...
        yAxis:[
            { title:{ text:"결과값", rotation:0 }, labels:{ align:"right", x:-10 }, 
              min:0, max:18, 
              tickAmount: 6,   
              tickPositions: [8, 10, 12, 14, 16, 18],          
        ...
        series:[
            { name:'진합', data:ash_result, yAxis:0 },
            { name:'KCC', data:ash_kcc, yAxis:0 }
        ]
    });
}
...
function fetchData3() {
    $.ajax({
        ...
        success:function(result){
            ...
            nv_kcc=uniqueOrder.map(rt=>safeNum(grouped[rt].nv_kcc));

            const newSeriesData=[ nv_result, nv_kcc];
	...
	kccNvChart = Highcharts.chart('kccNvChart',{
        ...
        yAxis:[
            { title:{ text:"결과값", rotation:0 }, labels:{ align:"right", x:-10 }, 
              min:0, max:23, 
              tickAmount: 7,   
              tickPositions: [11, 13, 15, 17, 19, 21, 23],          
              minorTickInterval: null, 
              endOnTick: true,
              maxPadding: 0,
              plotLines: [
        ...
        series:[
            { name:'진합', data:nv_result, yAxis:0 },
            { name:'KCC', data:nv_kcc, yAxis:0 }
        ]
    });
}
...
function fetchData4() {
    $.ajax({
        ...
        success:function(result){
            ...
            ph_kcc=uniqueOrder.map(rt=>safeNum(grouped[rt].ph_kcc));

            const newSeriesData=[ ph_result, ph_kcc];
	...
	kccPhChart = Highcharts.chart('kccPhChart',{
        ...
        yAxis:[
            { title:{ text:"결과값", rotation:0 }, labels:{ align:"right", x:-10 }, 
              min:0, max:6.8, 
              tickAmount: 9,   
              tickPositions: [5.2, 5.4, 5.6, 5.8, 6.0, 6.2, 6.4, 6.6, 6.8],          
              minorTickInterval: null, 
              endOnTick: true,
              maxPadding: 0,
              plotLines: [
        ...
        series:[
            { name:'진합', data:ph_result, yAxis:0 },
            { name:'KCC', data:ph_kcc, yAxis:0 }
        ]
    });
}
...
function fetchData5() {
    $.ajax({
        ...
        success:function(result){
            ...
            conductivity_kcc=uniqueOrder.map(rt=>safeNum(grouped[rt].conductivity_kcc));

            const newSeriesData=[ conductivity_result, conductivity_kcc];
...
function getTrend5(){
	kccConductivityChart = Highcharts.chart('kccConductivityChart',{
        chart:{ type:'line' },
        title:{ text:'Conductivity(μs/cm)<br>(Spec.1000~1800)' },
        plotOptions: {
            line: { // 혹은 series: 로 작성 가능
                marker: {
                    enabled: true // 마커(도형) 표시 안 함
        ...
        series:[
            { name:'진합', data:conductivity_result, yAxis:0 },
            { name:'KCC', data:conductivity_kcc, yAxis:0 }
        ]
    });
}
