    ...
    $(function () {
    	 var now    = new Date();
    	    ...
    	    function resetCleanForm() {
    	        $("#cleanForm")[0].reset();     // form 기본값으로 리셋 (O로 돌아감)
    	        $('input[name="idx"]').val(""); // 수정용 idx 초기화
    	    }
    ...
    function getDataList2() {
        ...
        dataTable = new Tabulator("#dataList2", {
            ...
            rowDblClick: function (e, row) {
                ...
                // ✅ 값이 없으면 무조건 "O"로 처리
                $('select[name="a_1"]').val(data.a_1 ? data.a_1 : "O");
                $('select[name="a_2"]').val(data.a_2 ? data.a_2 : "O");
                $('select[name="a_3"]').val(data.a_3 ? data.a_3 : "O");
                $('select[name="a_4"]').val(data.a_4 ? data.a_4 : "O");
                $('select[name="a_5"]').val(data.a_5 ? data.a_5 : "O");
